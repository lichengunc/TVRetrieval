"""
For both SVMR and VR, we will use the fused [video+subtitle] representations.
"""
import math
import copy
import torch
...
class XML_EARLY(nn.Module):
    def __init__(self, config):
        super(XML_EARLY, self).__init__()
        self.config = config
        ...
        self.query_input_proj = LinearLayer(config.query_input_size,
                                            ...
                                            relu=True)
        # we only consider using transformer as encoder
        assert self.config.encoder_type == "transformer", \
            f"Current version does not support {self.config.encoder_type}."
        ...
        # query vectors
        # we only have 1 module, i.e., query_joint
        ...
        assert config.merge_two_stream and config.span_predictor_type == "conv"
    ...
    def get_pred_from_raw_query(self, query_feat, query_mask, 
                                joint_feat, joint_mask, cross=False):
        """
        Args
            query_feat: (N, Lq, Dq)
            query_mask: (N, Lq)
            ...
            cross     : bool
        """
        joint_query = self.encode_query(query_feat, query_mask)  # (N, D)
        # get video-level retrieval scores
        q2ctx_scores = self.get_video_level_scores(
                            joint_query, joint_feat, joint_mask) # (N, N)
        # get st_prob and ed_prob
        st_prob, ed_prob = self.get_merged_st_ed_prob(
            joint_query, joint_feat, joint_mask, cross=cross)
    ...
    def encode_query(self, query_feat, query_mask):
        ...
        joint_query = \
            self.get_modularized_queries(encoded_query, query_mask)  # (N, D) * 1
        return joint_query
